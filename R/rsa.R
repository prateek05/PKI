...
PKI.load.private.pem <- function(Location){
	private <- TRUE
	either <- (length(private) > 0) && isTRUE(all(is.na(private)))
	private <- isTRUE(private)	
	
	if (!missing(Location)) {
			what <- con <- file(Location, "r")
			on.exit(close(con))
		}
	
	
	#what <- readPEM(Location)
		if (inherits(what, "connection"))
			what <- readLines(what)
		if (is.character(what)) {
			## remove any lines containing : or ending in \ which are headers
...
PKI.sign.PEM <- function(what, key, hash=c("SHA1", "MD5"), digest) {
	strToPrivateKey <- function(key){
		key <- base64enc::base64decode(key)
		.Call(PKI_load_private_RSA, key)
	}
	
	what <- charToRaw(what)
	key <- strToPrivateKey(key)
	if (!missing(digest) && !missing(what))
		stop("what and digest are mutually exclusive")
	if (missing(digest))
		digest <- PKI.digest(what, hash)
